...
const Temp6 = ({ data }) => {
  const montantEnLettres = writtenNumber(Math.round(data.total || 0));

  return (
    <div className="bg-white font-sans p-8  max-w-4xl mx-auto shadow-lg">
      {/* HEADER */}
      <div className="flex justify-between items-center border-b border-purple-300 pb-4 mb-6">
        <div className="flex items-center space-x-4">
          {data.companyLogo && <img src={data.companyLogo} alt="Logo" className="h-25 w-25  object-cover" />}
          <h1 className="text-xl font-accent text-gray-600">{data.companyName}</h1>
        </div>
        <div className="text-right">
          <h2 className="text-lg font-bold text-purple-700 uppercase">{data.title || 'FACTURE'}</h2>
      ...
      {/* COMPANY & CLIENT INFO */}
      <div className="flex justify-between mb-6 p-4 bg-purple-50 rounded-lg text-purple-700">
        <div>
      ...
      {/* OBJET */}
      {data.serviceSubject && (
        <div className="mb-4 p-2 border-l-4 border-purple-600 bg-purple-100 rounded">
      ...
      <div className="flex flex-col items-end mb-8 space-y-3">
        <div className="w-80 text-sm">
          ...
          {/* Montant TTC */}
          <div className="flex justify-between py-3 bg-purple-600  text-white font-bold rounded-lg mt-2 shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          {(data.accountMethod || data.accountNumber || data.accountRIB) && (
            <div className="flex-1">
              <h3 className="text-md font-bold bg-purple-600 text-white p-2 rounded-t-md">Modalités de paiement</h3>
          ...
          (data.remainingPercent && data.remainingAmount && data.remainingDate) ? (
            <div className="flex-1">
              <h3 className="text-md font-bold bg-purple-600 text-white p-2 rounded-t-md">Conditions de règlement</h3>
      ...
      <div className="text-xs text-gray-600 mb-2 text-center">
        <p className="text-sm font-accent text-purple-600 text-center"><strong>Merci pour la confiance que vous nous accordez !</strong></p>
      </div>
